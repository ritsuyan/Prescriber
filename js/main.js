...
/*   // 05/09
 ...
 todo 2  the choose column page cannot tap btn       = > complete
...
/**************************************     sort   data    start   *********************************/
add_sort({
   'dom' : document.getElementById('swipe_table_title'),
    'add_dom' : document.getElementById('swiper-body')
})
function add_sort(opts) {
    var dom = opts.dom;
    var td_arr = opts.add_dom.getElementsByTagName('tr')[0].getElementsByTagName('td');
    var th_arr = dom.getElementsByTagName('th');
    var len = th_arr.length;

    for (var i = 0; i < len; i++) {
        if (parseInt(td_arr[i]) !== NaN ) {
            var curr_hammer = new Hammer(th_arr[i]);

            curr_hammer.on('doubletap', function (ev) {
    ...
    console.log($index)
        ...
        for (var x = 0; x < rows1.length - 1; x++) {
            for (var y = x + 1; y < rows1.length; y++) {
                console.log(parseInt(rows1[x].cells[$index].innerHTML))
        ...
        if (is_sort === false) {
            for (var x = 0; x < rows1.length; x++) {
                //       tabNode.childNodes[0].appendChild(rows[x]);
...
function toggle_btn(){
    ...
    for (var i = 0; i < 3; i++) {
        var hammer_btn = new Hammer(btn_arr[i]);
...
function show_btn(opts){
    var $pa = opts.dom;
    var $btn_arr = $pa.getElementsByClassName('fun_btn');
    var $btn_arr_len = $btn_arr.length;

    if(is_show === false) {

        for(var i = 0 ; i < $btn_arr_len; i++){
            $btn_arr[i].className = 'fun_btn show_btn';
        }
        return true;
    } else{
        for(var i = 0 ; i < $btn_arr_len; i++){
            $btn_arr[i].className = 'fun_btn hide_btn';
        }
        return false;
    }
...
function cre_view(opts){
   // if(document.getElementById('fixed_table_title') !== null ){
...
function filter_fun(opts){
  ...
  //  $(td_arr).remove()
    ...
    var dom_tr = dom.getElementsByTagName('tr');
    ...
    for(var y = arr.length ; y < dom_tr.length + 1; y++){
        $(dom_tr[y]).remove()
    }
...
function delete_tr_2() {
    if( document.getElementById('swiper-body') !== null){
...
//setInterval('checked_tr_len()',100)

function checked_tr_len () {
    var rr = document.getElementById('swiper-body');
    if(rr !== undefined){
        var dom_tr = document.getElementById('swiper-body').getElementsByTagName('tr');
        var dom_len = dom_tr.length;
        var add_arr = document.getElementById('fixed-body').getElementsByTagName('td');
        var add_len = add_arr.length;
        console.log(dom_tr)
        console.log(dom_len)   //3
        console.log(add_len)   // 2

        if(dom_len > add_len){
            var su =  dom_len - add_len;  // 1

            for(var i = add_len; i < add_len + su ; i ++){
               $(dom_tr[i]).remove()
            }
        }
        console.log('delete')
